...
from srcs.utils.nn import KDModule, get_optimizer, ChannelCombine
...
class SIGN(pl.LightningModule):
    def __init__(self, config, dataset, device, steps_per_epoch):
        ...
        self.kd_teacher = KDModule(config.teacher_path, config.temperature, device)
        self.alpha = self.config['kd_alpha']
    ...
    def training_step(self, batch, batch_idx):
        ...
        kd_loss = self.kd_teacher.kd_loss(logits, y, nid)
        loss = (1 - self.alpha) * ce_loss + self.alpha * kd_loss
        self.train_acc(torch.softmax(logits, 1), y)
        self.log('train_loss', ce_loss, prog_bar=True, on_epoch=True)
        self.log('train_total_ce', (1 - self.alpha) * ce_loss, prog_bar=True, on_epoch=True)
        self.log('train_total_kd', self.alpha * kd_loss, prog_bar=True, on_epoch=True)
        self.log('train_total_loss', loss, prog_bar=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_loss', ce_loss, prog_bar=True, on_epoch=True)
        self.log('val_total_ce', (1 - self.alpha) * ce_loss, prog_bar=True, on_epoch=True)
        self.log('val_total_kd', self.alpha * kd_loss, prog_bar=True, on_epoch=True)
        self.log('val_total_loss', loss, prog_bar=True, on_epoch=True)
